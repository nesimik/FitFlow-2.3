...
object Palette {
    ...
    val warning = Color(0xFFE0A33A)
    val danger = Color(0xFFE0584F)
...
fun parseHex(hex: String, fallback: Color = Color(0xFF3CD3EB)): Color = try {
...
fun onColorFor(bg: Color): Color {
    ...
    return if (l > 0.55f) Color(0xFF03161B) else Color.White
}
...
val LocalFitColors = staticCompositionLocalOf {
    FitColors(
        accent = Color(0xFF3CD3EB), onAccent = Color(0xFF03161B), success = Palette.success,
...
/**
 * Tasarım dili: ağırlık, tekrar, süre, yüzde gibi tüm sayılar eş genişlikli (mono) yazıyla
 * gösterilir; böylece alt alta gelen değerler hizalı durur ve "ölçüm" hissi verir.
 */
val MonoFamily: FontFamily = FontFamily.Monospace

/** Bir yazı stilini mono sayı stiline çevirir: `style = MaterialTheme.typography.titleSmall.mono()` */
fun TextStyle.mono(): TextStyle = copy(fontFamily = MonoFamily, letterSpacing = 0.sp)

/** Küçük büyük harfli üst etiket stili ("BUGÜN · ÜST VÜCUT"). */
fun TextStyle.overline(): TextStyle = copy(fontFamily = MonoFamily, fontWeight = FontWeight.Bold, letterSpacing = 1.1.sp)
